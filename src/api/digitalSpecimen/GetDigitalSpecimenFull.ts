...
/**
 * Function for fetching a digital specimen and all of its associating information
 ...
 */
const GetDigitalSpecimenFull = async({ handle, version } : { handle: string, version?: number }) => {
    ...
    if (handle) {
        ...
        if (version) {
            endPoint = `digital-specimen/${handle.replace(import.meta.env.VITE_DOI_URL, '')}/${version}/full`;
        } else {
            endPoint = `digital-specimen/${handle.replace(import.meta.env.VITE_DOI_URL, '')}/full`;
        };
...
export default GetDigitalSpecimenFull;