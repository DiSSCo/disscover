...
/** Component that renders a default dropdown with selectable items, each item will trigger an action
    ...
    * @param selectedItemValue The currently selected item, holds a label and value
...
const Dropdown = (props: Props) => {
    const { items, selectedItemValue, placeholder, styles, OnChange } = props;
    ...
    if (placeholder && !selectedItemValue && !items.find((item) => [placeholder, 'Reset filter'].includes(item.label))) {
        items.unshift({
            label: placeholder,
            value: ""
        });
    ...
    const CheckItems = (option: SingleValue<DropdownItem>) => {
        if (option?.value && !selectedItemValue && !selectItems.find((item) => item.label === 'Reset filter')) {
    ...
    return (
        <Select
            value={selectItems.find(item => item.value === selectedItemValue)}
            defaultValue={{
